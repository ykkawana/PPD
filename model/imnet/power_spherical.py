...
class HypersphericalUniform(torch.distributions.Distribution):
    ...
    def rsample(self, sample_shape=()):
        v = torch.empty(sample_shape + (self.dim, ),
                        device=self.device,
                        dtype=self.dtype).normal_()
        return v / (v.norm(dim=-1, keepdim=True) + _EPS)

    def log_prob(self, value):
        return torch.full_like(
            value[..., 0],
            math.lgamma(self.dim / 2) - (math.log(2) +
...
class MarginalTDistribution(torch.distributions.TransformedDistribution):
    ...
    def __init__(self, dim, scale, validate_args=None):
        super().__init__(
            torch.distributions.Beta((dim - 1) / 2 + scale, (dim - 1) / 2,
...
class _JointTSDistribution(torch.distributions.Distribution):
    ...
    def rsample(self, sample_shape=()):
        return torch.cat(
            (
                ...
                self.marginal_s.rsample(sample_shape +
                                        self.marginal_t.scale.shape),
            ),
            -1,
        )
    ...
    def log_prob(self, value):
        return self.marginal_t.log_prob(
            value[..., 0]) + self.marginal_s.log_prob(value[..., 1:])
...
class PowerSpherical(torch.distributions.TransformedDistribution):
    ...
    def __init__(self, loc, scale, validate_args=None):
        ...
        super().__init__(
            _JointTSDistribution(
                MarginalTDistribution(loc.shape[-1],
                                      scale,
                                      validate_args=validate_args),
    ...
    def log_normalizer(self):
        ...
        return -((alpha + beta) * math.log(2) + torch.lgamma(alpha) -
                 torch.lgamma(alpha + beta) + beta * math.log(math.pi))

    def entropy(self):
        alpha = self.base_dist.marginal_t.base_dist.concentration1
        beta = self.base_dist.marginal_t.base_dist.concentration0
        return -(
            self.log_normalizer() + self.scale *
